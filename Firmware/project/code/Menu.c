...
static void PIDEdit_Init(void)
{
    ...
    // 根据我的实际变量修改
    pid_edit_state.kp_ptr = &Config.motor_l.Kp;
    pid_edit_state.ki_ptr = &Config.motor_l.Ki;
    pid_edit_state.kd_ptr = &Config.motor_l.Kd;
}

static void PIDEdit_Start(void)
{
    // 保存原始值
    if (pid_edit_state.kp_ptr)
        original_kp = *pid_edit_state.kp_ptr;
    if (pid_edit_state.ki_ptr)
        original_ki = *pid_edit_state.ki_ptr;
    if (pid_edit_state.kd_ptr)
        original_kd = *pid_edit_state.kd_ptr;
...
static void PIDEdit_Stop(uint8_t save_changes)
{
    if (!save_changes)
    {
        // 恢复原始值
        if (pid_edit_state.kp_ptr)
            *pid_edit_state.kp_ptr = original_kp;
        if (pid_edit_state.ki_ptr)
            *pid_edit_state.ki_ptr = original_ki;
        if (pid_edit_state.kd_ptr)
            *pid_edit_state.kd_ptr = original_kd;
    }
...
static void PIDEdit_AdjustValue(float delta)
{
    ...
    {
    case 0:
        target = pid_edit_state.kp_ptr;
        break;
    case 1:
        target = pid_edit_state.ki_ptr;
        break;
    case 2:
        target = pid_edit_state.kd_ptr;
        break;
    }
...
void Menu_DisplayRealtimeParams(void)
{
    ...
    case 0: // 第1页：电池和电机状态
    {
        ...
        OLED_ShowFloatNum(60, line * 8, State.motor_target_speed_left, 3, 3, OLED_6X8);
        line++;
        // 左电机实际速度
        OLED_ShowString(0, line * 8, "L-Actual:", OLED_6X8);
        OLED_ShowFloatNum(60, line * 8, State.motor_actual_speed_left, 3, 3, OLED_6X8);
        line++;
        // 右电机目标速度
        OLED_ShowString(0, line * 8, "R-Target:", OLED_6X8);
        OLED_ShowFloatNum(60, line * 8, State.motor_target_speed_right, 3, 3, OLED_6X8);
        line++;
        // 右电机实际速度
        OLED_ShowString(0, line * 8, "R-Actual:", OLED_6X8);
        OLED_ShowFloatNum(60, line * 8, State.motor_actual_speed_right, 3, 3, OLED_6X8);
        line++;
        break;
    }
    ...
    case 3: // 第4页：可调控PID参数
    {
        ...
        OLED_ShowFloatNum(50, line * 8, Config.motor_l.Kp, 3, 2, OLED_6X8);
        line++;
        ...
        OLED_ShowFloatNum(50, line * 8, Config.motor_l.Ki, 3, 2, OLED_6X8);
        line++;
        ...
        OLED_ShowFloatNum(50, line * 8, Config.motor_l.Kd, 3, 2, OLED_6X8);
        line++;
