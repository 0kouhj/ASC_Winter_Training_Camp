...
#define KEY1                    (E2 )                                           // 使用的外部中断输入引脚 如果修改 需要同步对应修改外部中断编号与 isr.c 中的调用
#define KEY2                    (E3 )                                           // 使用的外部中断输入引脚 如果修改 需要同步对应修改外部中断编号与 isr.c 中的调用
#define KEY3                    (E4 )                                           // 使用的外部中断输入引脚 如果修改 需要同步对应修改外部中断编号与 isr.c 中的调用
#define KEY4                    (E5 )                                           // 使用的外部中断输入引脚 如果修改 需要同步对应修改外部中断编号与 isr.c 中的调用

#define KEY1_EXTI               (EXTI2_IRQn  )                                  // 对应外部中断的中断编号 在 mm32f3277gx.h 头文件中查看 IRQn_Type 枚举体
#define KEY2_EXTI               (EXTI3_IRQn  )                                  // 对应外部中断的中断编号 在 mm32f3277gx.h 头文件中查看 IRQn_Type 枚举体
#define KEY3_EXTI               (EXTI4_IRQn  )                                  // 对应外部中断的中断编号 在 mm32f3277gx.h 头文件中查看 IRQn_Type 枚举体
#define KEY4_EXTI               (EXTI9_5_IRQn)                                  // 对应外部中断的中断编号 在 mm32f3277gx.h 头文件中查看 IRQn_Type 枚举体
...
int main (void)
{
    ...
    while(1)
    {
        ...
        if(exti_state[0])
        {
            for(loop_time = 1; 0 < loop_time; loop_time --)
            {
        ...
        {
            for(loop_time = 2; 0 < loop_time; loop_time --)
            {
                gpio_toggle_level(LED1);                                        // 翻转 LED 引脚输出电平 控制 LED 亮灭
                system_delay_ms(100);                                           // 延时
                gpio_toggle_level(LED1);                                        // 翻转 LED 引脚输出电平 控制 LED 亮灭
        ...
        {
            for(loop_time = 3; 0 < loop_time; loop_time --)
            {
                gpio_toggle_level(LED1);                                        // 翻转 LED 引脚输出电平 控制 LED 亮灭
                system_delay_ms(100);                                           // 延时
                gpio_toggle_level(LED1);                                        // 翻转 LED 引脚输出电平 控制 LED 亮灭
        ...
        {
            for(loop_time = 4; 0 < loop_time; loop_time --)
            {
                gpio_toggle_level(LED1);                                        // 翻转 LED 引脚输出电平 控制 LED 亮灭
                system_delay_ms(100);                                           // 延时
                gpio_toggle_level(LED1);                                        // 翻转 LED 引脚输出电平 控制 LED 亮灭
